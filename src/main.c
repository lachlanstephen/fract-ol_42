...
/*   Updated: 2024/03/30 23:15:59 by lstephen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	param_checker(char **argv, int argc)
{
	...
	i = 1;
	(void)argc;
	if ((argv[i][0] != 'm' && argv[i][0] != 'j') || (argv[i][0] == 'm' && argc > 2))
		list_params();
}
...
int	main(int argc, char *argv[])
{
	...
	img.address = mlx_get_data_addr(img.img_ptr, &img.bits_per_pixel, &img.line_length, &img.endian);
	if (argc < 2 || argc > 4)
		list_params();
	...
	{
		 if (argc == 3)
			ft_julia(ft_atof_fractol(argv[2]), 0, &img, &vars);
		else if (argc == 4)
			ft_julia(ft_atof_fractol(argv[2]), ft_atof_fractol(argv[3]), &img, &vars);
		else
			ft_julia(0, 0, &img, &vars);
	}
